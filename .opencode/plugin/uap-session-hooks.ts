    ...
    event: async ({ event }) => {
      if (event.type === 'session.created') {
        try {
          ...
          // UAP Compliance Protocol
          console.log('\n--- UAP COMPLIANCE PROTOCOL ---');
          console.log('1. Query memory before starting work');
          console.log('2. Use worktrees for isolation');
          console.log('3. Store learnings before session ends');
          console.log('4. Check for open loops from previous sessions');
          console.log('--- END PROTOCOL ---\n');
        } catch {
          /* fail safely */
        }
      }
