...
class TestRunnerSimulator {
  ...
  runAllTests(): { passed: number; failed: number; results: Array<{ name: string; passed: boolean }> } {
...
Make sure to complete steps in the correct order!`,
    ...
    verify: (async () => {
      ...
      try {
        ...
        // ignore parse errors for benchmark verification
      }
      ...
      const allComplete = Boolean(step1Complete) && step2Complete;
...
Remember: These functions follow the pattern function name(a: number, b: number): number.`,
    ...
    verify: (async () => {
      ...
      if (!hasMultiplyTest) return { success: false, details: { reason: 'Missing multiply() test' } };
