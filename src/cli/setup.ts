...
function printSummary(cwd: string, qdrantReady: boolean, pythonPath: string | null): void {
  const checks = [
    { label: 'Created .uam.json', ok: existsSync(join(cwd, '.uam.json')) },
    { label: 'Directory structure ready', ok: existsSync(join(cwd, 'agents/data/memory')) },
    { label: 'Memory database initialized', ok: existsSync(join(cwd, 'agents/data/memory/short_term.db')) },
    { label: 'Generated CLAUDE.md', ok: existsSync(join(cwd, 'CLAUDE.md')) || existsSync(join(cwd, 'AGENT.md')) },
    { label: 'Qdrant available', ok: qdrantReady },
    { label: 'Python venv ready', ok: !!pythonPath },
    { label: 'Pattern scripts generated', ok: existsSync(join(cwd, 'agents/scripts/index_patterns_to_qdrant.py')) },
  ];
