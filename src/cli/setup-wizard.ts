...
async function promptModelConfig(): Promise<ModelConfig> {
  ...
  if (provider === 'local') {
    const qwenAnswer = await inquirer.prompt<{ qwenOptimizations: boolean }>([
      {
        type: 'confirm',
        name: 'qwenOptimizations',
        message: 'Enable Qwen3.5 optimizations?',
        default: true,
      },
    ]);
    qwenOptimizations = qwenAnswer.qwenOptimizations;

    if (qwenOptimizations) {
      console.log('');
      console.log(chalk.cyan('  Qwen3.5 optimizations include: tool call reliability fixes,'));
      console.log(chalk.cyan('  chat template patches, llama.cpp parameter optimization for'));
      console.log(chalk.cyan('  16/24GB VRAM.'));
      console.log('');
      console.log(
        chalk.white('  Run ') +
          chalk.bold('uap-tool-calls') +
          chalk.white(' to apply fixes after setup.')
      );
      console.log('');
    }
  }
...
function printSummary(selections: WizardSelections): void {
  ...
  if (selections.model.provider === 'local') {
...
function printInstallPlan(selections: WizardSelections): void {
  ...
  if (selections.model.provider === 'local' && selections.model.qwenOptimizations) {
    steps.push('Flag Qwen3.5 tool-call fixes for post-install');
  }
...
function printFinalInstructions(selections: WizardSelections): void {
  ...
  if (selections.model.provider === 'local' && selections.model.qwenOptimizations) {
    postSteps.push('Run `uap-tool-calls` to apply Qwen3.5 tool-call fixes');
  }
  if (selections.browser.cloakBrowser) {
