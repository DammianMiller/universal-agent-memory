...
_uam_forgecode_session_start() {
  # Load recent memory context from UAM database before : command execution
  ...
  if command -v sqlite3 &> /dev/null; then
    ...
    if [ -n "\$OPEN_LOOPS" ]; then
      export UAM_OPEN_LOOPS="\$OPEN_LOOPS"  
...
_uam_forgecode_register() {
  # Add to ~/.zshrc if not already present (user confirmation required)
  local ZSHRC="\$HOME/.zshrc"
  
  if [ -f "\$ZSHRC" ] && ! grep -q "UAM_FORGE_CODE_PLUGIN" "\$ZSHRC"; then
...
EOF 
    echo "[UAP-ForgCode] ✅ Created .forge/settings.local.json with UAM integration config" >&2  
  fi
  ...
  _uam_forgecode_session_start &>/dev/null || true  
}

# Export functions so they can be sourced by other scripts
export -f _uam_forgecode_session_start \
           _uam_forgecode_pre_compact \
           _uam_forgecode_register;
