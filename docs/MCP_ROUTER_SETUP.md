...
# Show token savings
uam mcp-router stats

# Discover tools
uam mcp-router discover --query "github issues"

# Start as MCP server (for use with Claude/Cursor)
uam mcp-router start
```
...
```bash
# Create ~/.uam/mcp-backend.json
mkdir -p ~/.uam
cat > ~/.uam/mcp-backend.json << 'EOF'
{
  "mcpServers": {
    "playwright": {
      "command": "npx",
...
uam mcp-router start --config ~/.uam/mcp-backend.json
```
...
When run with `uam mcp-router start`, the router exposes 2 tools:
...
```bash
# Search all servers
uam mcp-router discover --query "github"

# Filter to specific server
uam mcp-router discover --query "create" --server github

# Limit results
uam mcp-router discover --query "file" --limit 3

# JSON output
uam mcp-router discover --query "search" --json
```
...
**Symptom**: `uam mcp-router stats` shows `0 tools`.
...
uam mcp-router stats --config ~/.uam/mcp-backend.json
```
...
- Forget to check token savings with `uam mcp-router stats`
...
```bash
# Initialize UAM in a project
uam init

# Memory query with router context
uam memory query "mcp router setup"
...
- **Issues**: https://github.com/your-org/universal-agent-memory/issues
- **Docs**: https://github.com/your-org/universal-agent-memory/docs
