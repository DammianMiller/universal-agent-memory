...
# universal-agent-protocol - Autonomous Agent Guide

> Universal AI agent protocol - CLAUDE.md templates, memory, worktrees for Claude Code, Factory.AI, VSCode, OpenCode
...
```bash
uap worktree create <slug>           # ALWAYS create first
cd .worktrees/NNN-<slug>/
git add -A && git commit -m "type: description"
uap worktree pr <id>                 # Create PR
# After PR merge:
uap worktree cleanup <id>            # MANDATORY cleanup after merge
```

**Applies to**: Application code, configs, workflows, documentation, CLAUDE.md itself — ALL changes without exception

**Cleanup is MANDATORY**: After every PR merge, immediately run `uap worktree cleanup <id>`. Never leave merged worktrees behind.
...
---
- **general, recommended**: Universal Agent Protocol is optimized for **opencode** - the local AI coding platform that provides:
...
- **general, store**: uap memory store "Always validate CSRF tokens in auth flows"
- **general, query**: uap memory query "auth security"
...
### Hot Spots
Frequently modified files (hot spots): package.json (77 changes), package-lock.json (42 changes), templates/CLAUDE.template.md (37 changes), .beads/issues.jsonl (33 changes), CLAUDE.md (23 changes), src/generators/claude-md.ts (12 changes), src/uap_harbor/uap_agent.py (12 changes), src/memory/adaptive-context.ts (9 changes), src/bin/cli.ts (9 changes), src/index.ts (9 changes). These files may need extra attention during changes.
